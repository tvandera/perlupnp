...
sub testSSDPEvent {
	...
	if (!$ssdpTestDone && ($port == NOTIFICATION_PORT) &&
		(inet_ntoa($iaddr) eq UPnP::Common::getLocalIPAddress())) {
		...
		is( $headers->header('Location'), 
			'http://' . UPnP::Common::getLocalIPAddress() . ':' . DEVICE_PORT . '/description.xml',
			'Location header' );
...
sub testControl {
	my $base = 'http://' . UPnP::Common::getLocalIPAddress() . ':' . DEVICE_PORT;
	...
	$req = HTTP::Request->new("SUBSCRIBE",
							...
							HTTP::Headers->new);
	$req->header('Host', UPnP::Common::getLocalIPAddress() . ':' . DEVICE_PORT);
	$req->header('Callback', '<http://' . UPnP::Common::getLocalIPAddress() . ':' 
				 . SUBSCRIPTION_PORT . '>');
...
sub testNotification {
	...
	if ($firstNotification) {
		...
		### Check the HOST header
		is ($req->header('HOST'), UPnP::Common::getLocalIPAddress() . ':' . SUBSCRIPTION_PORT, 
			'HOST header');
